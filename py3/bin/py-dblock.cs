...
""" #+begin_org
* ~[Summary]~ :: A =CSXU= for running py-dblock expansion on files.
#+end_org """
...
** This File: /bisos/git/auth/bxRepos/bisos-pip/startAiActivity/py3/bin/py-dblock.cs
...
#+end_org """
import typing
csInfo: typing.Dict[str, typing.Any] = { 'moduleName': ['py-dblock'], }
csInfo['version'] = '202507121200'
csInfo['status']  = 'inUse'
csInfo['panel'] = 'py-dblock-Panel.org'
...
***** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  Conventional top level example.
        ...
        cs.examples.menuChapter('=updateDblocks= -- expand all registered dblocks in files')
        cmnd('updateDblocks',
             args=['AI-DevStatus.org', 'AI-WorkPlan.org'],
             comment="# Expand all known dblocks in the listed files")

        return(cmndOutcome)


####+BEGIN: b:py3:cs:cmnd/classHead :cmndName "updateDblocks" :comment "Expand all registered dblocks in each given file" :extent "verify" :ro "cli" :parsMand "" :parsOpt "" :argsMin 1 :argsMax 9999 :pyInv ""
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  CmndSvc-   [[elisp:(outline-show-subtree+toggle)][||]] <<updateDblocks>>  =verify= argsMin=1 argsMax=9999 ro=cli   [[elisp:(org-cycle)][| ]]
#+end_org """
class updateDblocks(cs.Cmnd):
    cmndParamsMandatory = [ ]
    cmndParamsOptional = [ ]
    ...
    def cmnd(self,
             rtInv: cs.RtInvoker,
             cmndOutcome: b.op.Outcome,
             argsList: typing.Optional[typing.List[str]]=None,  # CsArgs
    ) -> b.op.Outcome:
...
####+END:
        self.cmndDocStr(f""" #+begin_org
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  Expand all registered dblocks in each given file.
For each file argument, runs all registered py-dblock handlers in a single pass.
Unrecognised dblock signatures are left untouched.
        #+end_org """)

        cmndArgs = self.cmndArgsGet("0&9999", cmndArgsSpecDict, argsList)
        if not cmndArgs:
            b_io.eh.problem_usageError("At least one file argument is required.")
            return failed(cmndOutcome)

        for fileArg in cmndArgs:
            filePath = pathlib.Path(fileArg)
            if not filePath.is_file():
                b_io.ann.note(f"SKIP (not a file): {filePath}")
                continue
            updateDblock.expandAll(filePath)
            b_io.ann.note(f"DBLOCK-UPDATED: {filePath}")

        return cmndOutcome.set(
            opError=b.op.OpError.Success,
            opResults=f"Processed {len(cmndArgs)} file(s).",
        )
...
####+END:
        ...
        cmndArgsSpecDict.argsDictAdd(
            ...
            argDefault='',
            argChoices=[],
            argDescription="One or more file paths to expand dblocks in.",
        )
