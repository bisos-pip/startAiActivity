...
- *Invariants*, symlinked from =mother/= (=CLAUDE.md=, =AI-AGENTS.org=,
...
which in turn imports =AI-AGENTS.org= (imports =AI-WORKFLOW.org= +
...
- *Customize for your own conventions* --- fork
  ...
  (CLAUDE.md contents, AI-AGENTS.org, AI-WORKFLOW.org) or add your own
...
** BISOS users: templates are pre-provisioned

Users on a BISOS system have the templates already available at
=/bisos/apps/defaults/ai-templates= (a git checkout of the same content,
maintained as part of the BISOS environment). If that path exists,
=startAiActivity.cs= auto-detects it as the default templates base. No
manual clone step needed.

* Post-Installation Setup

** For non-BISOS users: clone the templates repo, then configure

1. Clone the templates repo somewhere convenient:

   #+begin_src bash
   git clone https://github.com/bxexamples/aiActivityTemplates ~/aiActivityTemplates
   #+end_src

   (Path is your choice; =~/aiActivityTemplates= is a reasonable default.)

2. Tell =startAiActivity.cs= where the clone lives:

   #+begin_src bash
   startAiActivity.cs -i userConfig_set --parName=templates --parValue=~/aiActivityTemplates
   #+end_src
...
You are now ready to =initiate= projects. If you later want to customize
the templates for your own conventions, fork the repo, edit your fork,
and re-point =--parValue= at the fork.

** For BISOS users: nothing to do (usually)

If =/bisos/apps/defaults/ai-templates= exists on your system, it is used
as the default automatically. You can override it (to point at a personal
fork, for instance) with the same =userConfig_set= command shown above.
