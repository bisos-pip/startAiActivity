...
=userConfig_get=, and =userConfig_set= commands. Uses =bisos.b.userConfig_csu=
for persistent parameter management.

*** =py3/bin/py-dblock.cs=

A CS command that applies pure-Python org-mode dblock expansion to one or
more files. Imports =dblock_particulars= to register the
