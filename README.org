#+title: bisos.startAiActivity: Bootstrap an AI-Activity
#+DATE: <2026-07-12 Sat>
#+AUTHOR: Mohsen BANAN
#+OPTIONS: toc:4
...
|---------------------+-----------+-----------------------------------------------------------------------------|
| =userConfig_get=    | userConfig| Read a global persistent value (=~/.config/bisos/startAiActivity.cs/fps/=). |
| =userConfig_set=    | userConfig| Write a global persistent value. Typically used for =templates=.            |
| =cwdConfig_get=     | cwdConfig | Read a per-directory persistent value (=./.startAiActivity.cs/fps/=).       |
