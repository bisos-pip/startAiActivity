#+title: bisos.aiActivity: Bootstrap an AI-Activity
#+DATE: <2026-07-12 Sat>
#+AUTHOR: Mohsen BANAN
#+OPTIONS: toc:4
...
industrial default. What follows is the concrete mechanics of that
inversion --- and the templates tree is where the accumulated result
of that inversion lives.
...
install), =initiateSub= (slim subproject overlay), =aiSuspend= /
=aiResume= (reversible pause/restore), =refresh= (re-copy =CLAUDE.md= from
...
| Purple edge, numbered   | Claude reads this file. =1= = walk-up to CLAUDE.md; =2=--=5= = imports. |
...
- *Engine.* =aiActivity.cs= with its six operations. All
...
- [[#bisosstartaiactivity-is-a-command-only-pycs-facility][bisos.aiActivity is a Command-Only PyCS Facility]]
...
- [[#key-files][Key Files]]
    - [[#startaiactivitycs][aiActivity.cs]]
    - [[#pypi-and-github-packaging][PyPi and Github Packaging]]
- [[#documentation-and-blee-panels][Documentation and Blee-Panels]]
  - [[#bisosstartaiactivity-blee-panels][bisos.aiActivity Blee-Panels]]
- [[#support][Support]]
...
on your filesystem, then point =aiActivity.cs= at it via user config
...
- *Customize for your own conventions* --- fork
  ...
  activities, and point =aiActivity.cs= at your fork. Your changes
...
| =aiSuspend=     | Suspend AI collaboration --- stash editable files as =.dormant=, remove symlinks + =.claude/=. Reversible. |    |
| =aiResume=      | Restore a suspended install.                                               |                       |
| =deClaudify=      | Permanently remove all AI-collaboration files. Irreversible.               |                       |
...
invocations in the same directory (=refresh=, =aiSuspend= /
=aiResume=, or a second =initiate= after =deClaudify=) can run with
no CLI flags.
...
=.aiActivity.cs/fps/templates= override (see *Per-Directory Templates
...
=py3/bin/aiActivity.cs=: The main CS-MU (Command-Services Multi-Unit). Provides the =initiate=,
